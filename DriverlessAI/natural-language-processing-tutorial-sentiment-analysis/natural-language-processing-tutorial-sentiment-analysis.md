...
- [Task 5: Experiment Results Summary](#task-5-experimental-results-summary)
...
[Source](https://dsla1.com/sentiment-analysis-using-python/)
...
If you are using **Aquarium** as the environment, then the lab **Ana Castro's Driverless AI (1.7.0)** has the Amazon fine review dataset already pre-loaded with separate training and test datasets. The datasets can be located on the Datasets Overview page. However, you can also upload the datasets externally. To learn more about how to add the two datasets from the DAI file system then see [Appendix A: Add the Datasets.](https://codelabs-preview.appspot.com/?file_id=1iVTUYf1akGayP6i2yunNKa2mkdbs_d0CIUCyFVR-8MU#12)
...
![](assets/Task-1/Dropped-Columns.png)
...
![](assets/Task-1/final-experiment.png)
...
## Task 3: Natural Language Processing(NLP) Concepts
...
-   **Lemmatization :** A slight variant of stemming is lemmatization. The major difference between the two is that stemming can often create non-existent words, whereas lemmatization creates actual words. An example of Lemmatization: "run" is a base form for words like "running" or "ran" and the word "better" and "good" are in the same lemma so they are considered the same.

It is important to note here that the above steps are not mandatory and their usage depends upon the use case. For instance, in sentiment analysis emoticons signify polarity and stripping them off from the text may not be a good idea.
...
The intuition behind the Bag of Words is that documents are similar if they have similar content and we can get an idea about the meaning of the document from its content alone. For example, if our corpus contains the words {Learning, is, the, not, great}, and we want to vectorize the text "Learning is great", we would have the following vector: (1, 1, 0, 0, 1). 
...
## 6. Task 4: Driverless AI - NLP Recipe
...
**6. Convolution neural network models on:**
...
[5]  [Convolutional Neural Networks for Sentence Classification](https://arxiv.org/pdf/1408.5882.pdf)

[6]  [Text Classification, Part 2 - sentence-level Attentional RNN](https://richliao.github.io/supervised/classification/2016/12/26/textclassifier-RNN/)
...
Please note that the lab also provides a pre-ran experiment. You can either wait for your experiments to finish or use thr results of the pre-ran experiment. Click on the **Experiments** tab and select the Amazon Fine Food reviews experiment as follows:
...
-   1. To upload a custom recipe, Click on the UPLOAD CUSTOM RECIPE tab and select the desired recipe. Click save when done.
    

![](assets/Task-6/Uploading-Recipe-1.png)

  
-   2 Alternately, you can also upload a recipe via URL. Click on the LOAD CUSTOM RECIPE FROM URL tab and enter the raw Github URL of the recipe. Click save when done
...
3. Once the recipe is uploaded, the following screen will appear. Driverless AI automatically performs basic acceptance tests for all custom recipes unless disabled.
4. 
![](assets/Task-6/Acceptance-tests.png)

5. Click on Recipe and select or deselect specific transformers, models and scorers.![](assets/Task-6/Selecting-Specific-Transformers.png)
