...
In this tutorial, we will build a machine learning model using an example financial dataset and explore some of the most leading methods for explaining and interpreting that model. We will learn to interpret the results, graphs, scores and reason  code values of H2O Driverless AI generated models.
...
For this exercise, we will make use of the default of credit card clients Data Set. 
...
1. Load the UCI_Credit_Card.csv to Driverless AI by clicking **Add Dataset (or Drag and Drop)** on the **Datasets overview** page.  Click on the **UCI_Credit_Card.csv** file then select **Details**.

![task-1-data-details](assets/task-1-data-details.jpg)
...
8\. **Launch Experiment** and adjust the settings to **Accuracy:6**, **Time:4**, and **Interpretability:6**

![task-1-settings](assets/task-1-settings.jpg)

9\. While on the **Experiment** page:

![task-1-experiment-results](assets/task-1-experiment-results.jpg)
...
4. **Variable Importance** - Engineered features, we can see here a variety of automatically generated by Driverless automated feature engineering.

10\. Select **Interpret this Model** 

![task-1-interpret](assets/task-1-interpret.jpg)

11\. Once the **MLI Experiment is Finished** page comes up, select **Yes**, a landing page like this should appear

![task-1-landing-page](assets/task-1-landing-page.jpg)

12. If we click into the summary section we can scroll through the and learn the following.

![task-1-summary-1](assets/task-1-summary-1.jpg)

![task-1-summary-2](assets/task-1-summary-2.jpg)

![task-1-summary-3](assets/task-1-summary-3.jpg)

![task-1-summary-4](assets/task-1-summary-4.jpg)
...
![task-3-feature-importance](assets/task-3-feature-importance.jpg)
...
![task-3-rf-feature-importance](assets/task-3-rf-feature-importance.jpg)

This single **Random Forest** model of a complex Driverless AI model is very helpful because we can see  that this is a trustworthy model between the original inputs to the system and the predictions of the system. Note the low mean squared error(0.0327), high R2 (97%). 
...
3. PAY_0 =2 has an average default probability of 0.55 approximately, then the default probability slowly drops all the way to month 8. Does this make sense to you?
...
![task-4-pdp2](assets/task-4-pdp2.jpg)
...
![task-5-dt](assets/task-5-dt.jpg)
...
![task-5-dt-path](assets/task-5-dt-path.jpg)
...
![task-6-klime](assets/task-6-klime.jpg)
...
![task-6-klime-empty](assets/task-6-klime-empty.jpg)
...
![task-6-klime-targets](assets/task-6-klime-targets.jpg)
...
*Things to Note:*
1. The high value of R2=90% and low RMSE=0.0599 value show that this is a highly accurate linear model. In other words this surrogate model is good enough to proceed, and explains almost 90% of the variance in the Driverless AI model predictions.
...
![task-6-rc](assets/task-6-rc.jpg)

![task-6-rc-13](assets/task-6-rc-13.jpg)

*Things to Note:*

1. The reason codes show that the Driverless model prediction gave this person .61 percent probability of defaulting. LIME gave them a .69 percent probability of defaulting and in this case we can say that LIME is 88.4.2% accurate. Based on this observation, it can be concluded that the local reason codes are fairly trustworthy. If **Lime Prediction Accuracy** drops below 75% then we can say that the numbers are probably untrustworthy and the Shapley plot or LOCO plot should be revisited since the Shapley values are always accurate, and LOCO accounts for nonlinearity and interactions.
...
- Reason codes help us describe why the model made the decision it made for this specific person. Reason codes are particularly important in highly regulated industries when often regulators will want to see in as simple as possible terms “how did the model come to the conclusion it did”
...
![task-7-shap](assets/task-7-shap.jpg)
...
![task-8-dashboard](assets/task-8-dashboard.jpg)
...
- If a divergence is noted between ICE and **Partial Dependence** values, the **Decision Tree** can be used to learn more about the interactions between variables.
...
Be able to generate an MLI report, become familiar with the major features and walk through an example and be able to understand some of the key techniques and an example.
